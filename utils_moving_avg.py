...
def smooth_data(x, y, z, window_size=3):
    # Create a uniform filter
    filter = np.ones(window_size) / window_size

    # Apply convolution
    smooth_x = np.convolve(x, filter, mode='valid')
    smooth_y = np.convolve(y, filter, mode='valid')
    smooth_z = np.convolve(z, filter, mode='valid')
...
def main():
    ...
    for trial in controller_mapping:
        ...
        smooth_x, smooth_y, smooth_z = smooth_data(controller_user_curve_x, controller_user_curve_y,
                                                   controller_user_curve_z)

        # Plot the data
        plt.plot(smooth_x, smooth_y, smooth_z, label=f"controller_{count}", color="red", linewidth=1.0)
    ...
    for trial in hand_mapping:
        ...
        smooth_x, smooth_y, smooth_z = smooth_data(hand_user_curve_x, hand_user_curve_y, hand_user_curve_z)

        # Plot the data
        plt.plot(smooth_x, smooth_y, smooth_z, label=f"hand_{count}", color="green", linewidth=1.0)
    ...
    for trial in pen_mapping:
        ...
        smooth_x, smooth_y, smooth_z = smooth_data(pen_user_curve_x, pen_user_curve_y, pen_user_curve_z)

        plt.plot(smooth_x, smooth_y, smooth_z, label=f"pen_{count}", color="royalblue", linewidth=1.0)
