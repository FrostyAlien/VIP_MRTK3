...
def predict(x, y, z):
    ...
    # Create a 3D polynomial regression model
    model = make_pipeline(PolynomialFeatures(1), LinearRegression())

    model.fit(xyz, xyz)

    xyz_pred = model.predict(xyz)
...
def main():
    ...
    for trial in controller_mapping:
        ...
        xyz_pred = predict(controller_user_curve_x, controller_user_curve_y, controller_user_curve_z)

        # Plot the data
        plt.plot(xyz_pred[:, 0], xyz_pred[:, 1], xyz_pred[:, 2], label=f"controller_{count}", color="red", linewidth=1.0)
    ...
    for trial in hand_mapping:
        ...
        xyz_pred = predict(hand_user_curve_x, hand_user_curve_y, hand_user_curve_z)

        plt.plot(xyz_pred[:, 0], xyz_pred[:, 1], xyz_pred[:, 2], label=f"hand_{count}", color="green", linewidth=1.0)
    ...
    for trial in pen_mapping:
        ...
        xyz_pred = predict(pen_user_curve_x, pen_user_curve_y, pen_user_curve_z)

        # Plot the data
        plt.plot(xyz_pred[:, 0], xyz_pred[:, 1], xyz_pred[:, 2], label=f"pen_{count}", color="royalblue", linewidth=1.0)
