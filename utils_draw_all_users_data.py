...
def main():
    ...
    # create color map for different curve
    colors = ['red', 'green', 'blue', 'cyan', 'magenta', 'yellow', 'black', 'white', 'purple', 'orange']

    # loop over all users
    for USERID in all_users.keys():
        ...
        for device in ['Controller', 'Hand', 'Pen']:
            for mode in ['inner', 'outer']:
                ...
                # draw the user curve
                count = 0
                for trial in mapping:
                    user_curve_x = list()
                    user_curve_y = list()
                    user_curve_z = list()
                    for item in trial["mapping"]:
                        user_curve_x.append(item["user"]["x"])
                        user_curve_y.append(item["user"]["y"])
                        user_curve_z.append(item["user"]["z"])
                    plt.plot(user_curve_x, user_curve_y, user_curve_z, label=f"{device}_{count}", color=colors[count % len(colors)],
                             linewidth=0.8)
                    count += 1
